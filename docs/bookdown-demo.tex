...
\includegraphics{images/Ming_Tang_small.png}
...
\subsection{read in peak file}\label{read-in-peak-file}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{rownames}\NormalTok{(raw\_counts\_mat) }\OtherTok{\textless{}{-}}\NormalTok{ raw\_counts}\SpecialCharTok{$}\NormalTok{gene}
...
\NormalTok{map}\OtherTok{\textless{}{-}}\NormalTok{ AnnotationDbi}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(org.Hs.eg.db, }
                            \AttributeTok{keys =}\NormalTok{ exon\_len}\SpecialCharTok{$}\NormalTok{ENTREZID, }
                            \AttributeTok{columns=} \StringTok{"SYMBOL"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{table}\NormalTok{(}\FunctionTok{rownames}\NormalTok{(raw\_counts\_mat) }\SpecialCharTok{\%in\%}\NormalTok{ map}\SpecialCharTok{$}\NormalTok{SYMBOL)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{base}\SpecialCharTok{::}\FunctionTok{setdiff}\NormalTok{(}\FunctionTok{rownames}\NormalTok{(raw\_counts\_mat), map}\SpecialCharTok{$}\NormalTok{SYMBOL) }\SpecialCharTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{not\_in\_map}\OtherTok{\textless{}{-}} \FunctionTok{setdiff}\NormalTok{(}\FunctionTok{rownames}\NormalTok{(raw\_counts\_mat), map}\SpecialCharTok{$}\NormalTok{SYMBOL)}

\NormalTok{raw\_counts\_mat[not\_in\_map, ] }\SpecialCharTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{common\_genes}\OtherTok{\textless{}{-}} \FunctionTok{intersect}\NormalTok{(}\FunctionTok{rownames}\NormalTok{(raw\_counts\_mat), map}\SpecialCharTok{$}\NormalTok{SYMBOL)}

\DocumentationTok{\#\# select only the common genes and re{-}order them by common\_genes}
\NormalTok{map}\OtherTok{\textless{}{-}}\NormalTok{ map }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{slice}\NormalTok{(}\FunctionTok{match}\NormalTok{(common\_genes, SYMBOL))}

\CommentTok{\# subset the common genes and re{-}order them by common\_genes}
\NormalTok{raw\_counts\_mat}\OtherTok{\textless{}{-}}\NormalTok{ raw\_counts\_mat[common\_genes, ]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tpm }\OtherTok{\textless{}{-}} \FunctionTok{count2tpm}\NormalTok{(raw\_counts\_mat, map}\SpecialCharTok{$}\NormalTok{exon\_length)}

\FunctionTok{head}\NormalTok{(tpm)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mytest }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{t.test}\NormalTok{(x[}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{)], x[}\FunctionTok{c}\NormalTok{(}\DecValTok{3}\NormalTok{,}\DecValTok{4}\NormalTok{)], }\AttributeTok{var.equal =} \ConstantTok{TRUE}\NormalTok{)}\SpecialCharTok{$}\NormalTok{p.value}
\NormalTok{pvals }\OtherTok{\textless{}{-}} \FunctionTok{apply}\NormalTok{(tpm, }\DecValTok{1}\NormalTok{, mytest)}

\FunctionTok{head}\NormalTok{(pvals)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-35-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Define the order for boxplot}
\NormalTok{tpm\_df}\SpecialCharTok{$}\NormalTok{condition }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(tpm\_df}\SpecialCharTok{$}\NormalTok{condition, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Norm"}\NormalTok{, }\StringTok{"Hyp"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-41-1.pdf}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-42-1.pdf}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-43-1.pdf}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-45-1.pdf}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-49-1.pdf}
...
\hypertarget{analyzing-differential-gene-expression-with-deseq2}{%
\section{Analyzing Differential Gene Expression with DESeq2}\label{analyzing-differential-gene-expression-with-deseq2}}

In this step, we will explore the DESeq2 workflow, a widely used bioinformatics package in R for identifying differentially expressed genes (DEGs) from RNA sequencing (RNA-seq) data. \texttt{DESeq2} allows us to compare gene expression levels between different conditions or treatments and find genes that are significantly upregulated or downregulated.

\begin{quote}
\texttt{DESeq2} is an R package within the Bioconductor project that performs differential expression analysis on RNA-seq data. It uses a negative binomial distribution to model read counts and estimates the variance-mean dependence in the data to identify DEGs accurately. \texttt{DESeq2} is particularly useful when dealing with count data from RNA-seq experiments. You can read docs \href{https://bioconductor.org/packages/release/bioc/html/DESeq2.html}{here}.
\end{quote}

This Youtube video uses the same dataset and you may want to watch it if you prefer video.

\hypertarget{create-a-sample-sheet}{%
\subsection{Create a Sample Sheet}\label{create-a-sample-sheet}}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(DESeq2)}
\NormalTok{coldata }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{condition =} \FunctionTok{c}\NormalTok{(}\StringTok{"normoxia"}\NormalTok{, }\StringTok{"normoxia"}\NormalTok{, }\StringTok{"hypoxia"}\NormalTok{, }\StringTok{"hypoxia"}\NormalTok{))}
\FunctionTok{rownames}\NormalTok{(coldata) }\OtherTok{\textless{}{-}} \FunctionTok{colnames}\NormalTok{(raw\_counts\_mat)}
...
\hypertarget{create-a-deseq2-object}{%
\subsection{Create a DESeq2 Object}\label{create-a-deseq2-object}}

Next, we create a \texttt{DESeq2} object using the count data and the sample sheet. This object will store the count data and associated sample information and will be used for differential expression analysis by \texttt{DESeq2}.

The design formula specifies the experimental design, taking the condition as the main factor.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dds }\OtherTok{\textless{}{-}} \FunctionTok{DESeqDataSetFromMatrix}\NormalTok{(}\AttributeTok{countData =}\NormalTok{ raw\_counts\_mat,}
...
\hypertarget{get-differential-results}{%
\subsection{Get Differential Results}\label{get-differential-results}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{results}\NormalTok{(dds, }\AttributeTok{contrast =} \FunctionTok{c}\NormalTok{(}\StringTok{"condition"}\NormalTok{, }\StringTok{"hypoxia"}\NormalTok{, }\StringTok{"normoxia"}\NormalTok{))}
...
\hypertarget{explore-the-deseq2-results-and-create-visualizations.}{%
\subsection{Explore the DESeq2 results and create visualizations.}\label{explore-the-deseq2-results-and-create-visualizations.}}
...
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =} \FunctionTok{as.data.frame}\NormalTok{(res)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2FoldChange, }\AttributeTok{y =} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue))) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{14}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-54-1.pdf}
...
\hypertarget{examining-the-top-differentially-expressed-genes}{%
\subsection{Examining the Top Differentially Expressed Genes}\label{examining-the-top-differentially-expressed-genes}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{top\_differentially\_expressed\_genes }\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{arrange}\NormalTok{(padj, }\FunctionTok{desc}\NormalTok{(log2FoldChange)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{head}\NormalTok{(}\AttributeTok{n =} \DecValTok{30}\NormalTok{)}

\NormalTok{top\_differentially\_expressed\_genes}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{labeling-genes-in-the-volcano-plot}{%
\subsection{Labeling Genes in the Volcano Plot}\label{labeling-genes-in-the-volcano-plot}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{genes\_to\_label }\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  tibble}\SpecialCharTok{::}\FunctionTok{rownames\_to\_column}\NormalTok{(}\AttributeTok{var =} \StringTok{"gene"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(}\SpecialCharTok{!}\NormalTok{stringr}\SpecialCharTok{::}\FunctionTok{str\_detect}\NormalTok{(gene, }\StringTok{"LOC"}\NormalTok{),}
         \FunctionTok{abs}\NormalTok{(log2FoldChange) }\SpecialCharTok{\textgreater{}=} \FloatTok{2.5}\NormalTok{,}
\NormalTok{         padj }\SpecialCharTok{\textless{}=} \FloatTok{0.001}\NormalTok{)}

\FunctionTok{head}\NormalTok{(genes\_to\_label)}
\end{Highlighting}
\end{Shaded}
...
Now, we label these selected genes on the volcano plot using the \href{https://ggrepel.slowkow.com/}{\texttt{ggrepel}} package to prevent label overlap.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(ggrepel)}

\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =} \FunctionTok{as.data.frame}\NormalTok{(res), }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2FoldChange, }\AttributeTok{y =} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue))) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
\NormalTok{  ggrepel}\SpecialCharTok{::}\FunctionTok{geom\_label\_repel}\NormalTok{(}\AttributeTok{data =}\NormalTok{ genes\_to\_label, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{label =}\NormalTok{ gene)) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{14}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-57-1.pdf}

\hypertarget{coloring-points-in-the-volcano-plot}{%
\subsection{Coloring Points in the Volcano Plot}\label{coloring-points-in-the-volcano-plot}}
...
\NormalTok{res2 }\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  tibble}\SpecialCharTok{::}\FunctionTok{rownames\_to\_column}\NormalTok{(}\AttributeTok{var =} \StringTok{"gene"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{sig =} \FunctionTok{case\_when}\NormalTok{(}
    \SpecialCharTok{!}\NormalTok{stringr}\SpecialCharTok{::}\FunctionTok{str\_detect}\NormalTok{(gene, }\StringTok{"LOC"}\NormalTok{) }\SpecialCharTok{\&}
      \FunctionTok{abs}\NormalTok{(log2FoldChange) }\SpecialCharTok{\textgreater{}=} \FloatTok{2.5} \SpecialCharTok{\&}
\NormalTok{      padj }\SpecialCharTok{\textless{}=} \FloatTok{0.001} \SpecialCharTok{\textasciitilde{}} \StringTok{"sig"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(res2, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2FoldChange, }\AttributeTok{y =} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue))) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{color =}\NormalTok{ sig)) }\SpecialCharTok{+}
\NormalTok{  ggrepel}\SpecialCharTok{::}\FunctionTok{geom\_label\_repel}\NormalTok{(}\AttributeTok{data =}\NormalTok{ genes\_to\_label, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{label =}\NormalTok{ gene))}\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{14}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-59-1.pdf}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-60-1.pdf}
...
\FunctionTok{ggplot}\NormalTok{(res2, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ log2FoldChange, }\AttributeTok{y =} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue))) }\SpecialCharTok{+}
  ...
  \FunctionTok{scale\_color\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"blue"}\NormalTok{, }\StringTok{"red"}\NormalTok{)) }\SpecialCharTok{+}
\NormalTok{  ggrepel}\SpecialCharTok{::}\FunctionTok{geom\_label\_repel}\NormalTok{(}\AttributeTok{data =}\NormalTok{ genes\_to\_label, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{label =}\NormalTok{ gene))}\SpecialCharTok{+}
  \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =} \DecValTok{100}\NormalTok{, }\AttributeTok{linetype =} \DecValTok{2}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{2.5}\NormalTok{, }\FloatTok{2.5}\NormalTok{), }\AttributeTok{linetype =} \DecValTok{2}\NormalTok{, }\AttributeTok{color =} \StringTok{"red"}\NormalTok{)}\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{14}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-61-1.pdf}
...
\hypertarget{conclusion-34}{%
\subsection{Conclusion}\label{conclusion-34}}
...
\hypertarget{principal-component-analysis-pca-using-deseq2}{%
\section{Principal Component Analysis (PCA) using DESeq2}\label{principal-component-analysis-pca-using-deseq2}}
...
\hypertarget{what-is-principal-component-analysis-pca}{%
\subsection{What is Principal Component Analysis (PCA)?}\label{what-is-principal-component-analysis-pca}}
...
\hypertarget{why-use-pca}{%
\subsection{Why use PCA?}\label{why-use-pca}}
...
\hypertarget{using-deseq2-for-pca-analysis}{%
\subsection{Using DESeq2 for PCA Analysis}\label{using-deseq2-for-pca-analysis}}

DESeq2 includes a function called \texttt{plotPCA} for performing PCA analysis. However, in some cases, you may want to perform PCA manually for more customization. Let's walk through the steps.

DESeq2 has a \texttt{plotPCA} function to use:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#vsd is the normalized data}
...
\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-62-1.pdf}

\begin{itemize}
\item
  \texttt{dds} is assumed to be a DESeqDataSet object containing raw gene expression data.
\item
  \texttt{vst} stands for Variance Stabilizing Transformation, which is used to normalize the gene expression data. This transformation stabilizes the variance across samples, making the data suitable for PCA analysis.
\item
  The \texttt{blind=FALSE} argument indicates that the design of the experiment is not blinded.
\item
...
\hypertarget{plotting-pca-by-ourselves}{%
\subsection{Plotting PCA by ourselves}\label{plotting-pca-by-ourselves}}
...
\hypertarget{calculate-principal-components}{%
\subsubsection{Calculate Principal Components}\label{calculate-principal-components}}
...
In this step, we'll use the \texttt{prcomp} function to compute the principal components from the normalized gene expression data. The function takes the transpose of the normalized counts as input because PCA is typically performed on columns (samples) rather than rows (genes).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Get the normalized counts}
\NormalTok{normalized\_counts }\OtherTok{\textless{}{-}} \FunctionTok{assay}\NormalTok{(vsd) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{as.matrix}\NormalTok{()}

\CommentTok{\# Calculate the principal components}
\NormalTok{pca\_prcomp }\OtherTok{\textless{}{-}} \FunctionTok{prcomp}\NormalTok{(}\FunctionTok{t}\NormalTok{(normalized\_counts), }\AttributeTok{center =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{scale. =} \ConstantTok{FALSE}\NormalTok{)}

\FunctionTok{names}\NormalTok{(pca\_prcomp)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# the $x contains the PC loadings we want}
\NormalTok{pca\_prcomp}\SpecialCharTok{$}\NormalTok{x}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{create-a-dataframe-for-visualization}{%
\subsection{Create a DataFrame for Visualization}\label{create-a-dataframe-for-visualization}}
...
\NormalTok{PC1\_and\_PC2 }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{PC1 =}\NormalTok{ pca\_prcomp}\SpecialCharTok{$}\NormalTok{x[, }\DecValTok{1}\NormalTok{], }\AttributeTok{PC2 =}\NormalTok{ pca\_prcomp}\SpecialCharTok{$}\NormalTok{x[, }\DecValTok{2}\NormalTok{], }\AttributeTok{type =} \FunctionTok{rownames}\NormalTok{(pca\_prcomp}\SpecialCharTok{$}\NormalTok{x))}
\end{Highlighting}
\end{Shaded}

\hypertarget{plot-the-pca}{%
\subsection{Plot the PCA}\label{plot-the-pca}}

The final step is to create a PCA plot using the \texttt{ggplot2} package. This plot visually represents the relationships between samples in a reduced-dimensional space.
...
\CommentTok{\# Create a PCA plot}
\FunctionTok{ggplot}\NormalTok{(PC1\_and\_PC2, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ PC1, }\AttributeTok{y =}\NormalTok{ PC2, }\AttributeTok{col =}\NormalTok{ type)) }\SpecialCharTok{+} 
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} 
  \FunctionTok{geom\_text}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{label =}\NormalTok{ type), }\AttributeTok{hjust =} \DecValTok{0}\NormalTok{, }\AttributeTok{vjust =} \DecValTok{0}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{coord\_fixed}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-66-1.pdf}

\hypertarget{comparison-with-deseq2s-plotpca}{%
\subsection{Comparison with DESeq2's plotPCA}\label{comparison-with-deseq2s-plotpca}}
...
\hypertarget{creating-a-perfect-heatmap}{%
\section{Creating a Perfect Heatmap}\label{creating-a-perfect-heatmap}}
...
\hypertarget{selecting-significant-genes}{%
\subsection{Selecting Significant Genes}\label{selecting-significant-genes}}
...
\NormalTok{significant\_genes }\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(padj }\SpecialCharTok{\textless{}=} \FloatTok{0.01}\NormalTok{, }\FunctionTok{abs}\NormalTok{(log2FoldChange) }\SpecialCharTok{\textgreater{}=} \DecValTok{2}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{rownames}\NormalTok{()}

\FunctionTok{head}\NormalTok{(significant\_genes, }\DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The \texttt{significant\_genes} variable now contains the names of genes meeting these criteria.

\hypertarget{creating-the-heatmap}{%
\subsection{Creating the Heatmap}\label{creating-the-heatmap}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{significant\_mat }\OtherTok{\textless{}{-}}\NormalTok{ normalized\_counts[significant\_genes, ]}

\FunctionTok{Heatmap}\NormalTok{(}
  \FunctionTok{t}\NormalTok{(}\FunctionTok{scale}\NormalTok{(}\FunctionTok{t}\NormalTok{(significant\_mat))),}
  \AttributeTok{show\_row\_names =} \ConstantTok{FALSE}\NormalTok{,}
  \AttributeTok{name =} \StringTok{"scaled}\SpecialCharTok{\textbackslash{}n}\StringTok{expression"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-68-1.pdf}
...
\hypertarget{why-scaling-is-important}{%
\subsection{Why Scaling is Important?}\label{why-scaling-is-important}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{col\_anno }\OtherTok{\textless{}{-}} \FunctionTok{HeatmapAnnotation}\NormalTok{(}
  \AttributeTok{df =}\NormalTok{ coldata,}
  \AttributeTok{col =} \FunctionTok{list}\NormalTok{(}\AttributeTok{condition =} \FunctionTok{c}\NormalTok{(}\StringTok{"hypoxia"} \OtherTok{=} \StringTok{"red"}\NormalTok{, }\StringTok{"normoxia"} \OtherTok{=} \StringTok{"blue"}\NormalTok{))}
\NormalTok{)}

\FunctionTok{Heatmap}\NormalTok{(}
  \FunctionTok{t}\NormalTok{(}\FunctionTok{scale}\NormalTok{(}\FunctionTok{t}\NormalTok{(significant\_mat))),}
  \AttributeTok{top\_annotation =}\NormalTok{ col\_anno,}
  \AttributeTok{show\_row\_names =} \ConstantTok{FALSE}\NormalTok{,}
  \AttributeTok{name =} \StringTok{"scaled normalized}\SpecialCharTok{\textbackslash{}n}\StringTok{expression"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-69-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{c}\NormalTok{(}\StringTok{"hypoxia"} \OtherTok{=} \StringTok{"red"}\NormalTok{, }\StringTok{"normoxia"} \OtherTok{=} \StringTok{"blue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{the-impact-of-scaling}{%
\subsection{The Impact of Scaling}\label{the-impact-of-scaling}}
...
\begin{Highlighting}[]
\FunctionTok{Heatmap}\NormalTok{(}
\NormalTok{  significant\_mat,}
  \AttributeTok{top\_annotation =}\NormalTok{ col\_anno,}
  \AttributeTok{show\_row\_names =} \ConstantTok{FALSE}\NormalTok{,}
  \AttributeTok{name =} \StringTok{"normalized}\SpecialCharTok{\textbackslash{}n}\StringTok{expression"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-71-1.pdf}
...
\hypertarget{conclusion-36}{%
\subsection{Conclusion}\label{conclusion-36}}
...
Scaling is crucial for visualizing relative changes, and annotations provide context to help us interpret the heatmap effectively. Heatmaps are powerful tools for identifying trends and patterns within complex datasets, making them invaluable in fields like genomics and beyond. I highly recommend you to read the tutorial \url{https://jokergoo.github.io/ComplexHeatmap-reference/book/}. It is very comprehensive and we only covered a small number of features.

\hypertarget{pathway-analysis-using-over-representation-and-gene-set-enrichment-analysis}{%
\section{Pathway Analysis Using Over-Representation and Gene Set Enrichment Analysis}\label{pathway-analysis-using-over-representation-and-gene-set-enrichment-analysis}}
...
\hypertarget{understanding-pathway-analysis}{%
\subsection{Understanding Pathway Analysis}\label{understanding-pathway-analysis}}
...
For further understanding go here: \url{https://yulab-smu.top/biomedical-knowledge-mining-book/enrichment-overview.html}
...
\CommentTok{\#convert gene symbol to Entrez ID }

\NormalTok{significant\_genes\_map}\OtherTok{\textless{}{-}}\NormalTok{ clusterProfiler}\SpecialCharTok{::}\FunctionTok{bitr}\NormalTok{(}\AttributeTok{geneID =}\NormalTok{ significant\_genes,}
                      \AttributeTok{fromType=}\StringTok{"SYMBOL"}\NormalTok{, }\AttributeTok{toType=}\StringTok{"ENTREZID"}\NormalTok{,}
                      \AttributeTok{OrgDb=}\StringTok{"org.Hs.eg.db"}\NormalTok{)}

\FunctionTok{head}\NormalTok{(significant\_genes\_map)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# background genes are genes that are detected in the RNAseq experiment }
\NormalTok{background\_genes}\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(baseMean }\SpecialCharTok{!=} \DecValTok{0}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  tibble}\SpecialCharTok{::}\FunctionTok{rownames\_to\_column}\NormalTok{(}\AttributeTok{var =} \StringTok{"gene"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{pull}\NormalTok{(gene)}


\NormalTok{res\_df}\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(baseMean }\SpecialCharTok{!=} \DecValTok{0}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  tibble}\SpecialCharTok{::}\FunctionTok{rownames\_to\_column}\NormalTok{(}\AttributeTok{var =} \StringTok{"gene"}\NormalTok{)}

\NormalTok{background\_genes\_map}\OtherTok{\textless{}{-}} \FunctionTok{bitr}\NormalTok{(}\AttributeTok{geneID =}\NormalTok{ background\_genes, }
                            \AttributeTok{fromType=}\StringTok{"SYMBOL"}\NormalTok{, }
                            \AttributeTok{toType=}\StringTok{"ENTREZID"}\NormalTok{,}
                      \AttributeTok{OrgDb=}\StringTok{"org.Hs.eg.db"}\NormalTok{)}

\FunctionTok{head}\NormalTok{(background\_genes\_map)}
\end{Highlighting}
\end{Shaded}
...
The resulting \texttt{background\_genes\_map} object contains a mapping between gene symbols and their corresponding Entrez IDs for the background genes. This mapping is essential for subsequent gene ontology enrichment analysis to ensure that the analysis considers standardized gene identifiers.

\hypertarget{some-concepts-to-remember}{%
\subsection{Some concepts to remember}\label{some-concepts-to-remember}}
...
Now let's perform a Gene Ontology (GO) enrichment analysis using the \texttt{enrichGO} function from the \texttt{clusterProfile}r package. GO enrichment analysis is a widely used technique in bioinformatics to determine whether a set of genes is overrepresented in specific functional categories, such as biological processes (BP), cellular components (CC), or molecular functions (MF).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ego }\OtherTok{\textless{}{-}} \FunctionTok{enrichGO}\NormalTok{(}\AttributeTok{gene          =}\NormalTok{ significant\_genes\_map}\SpecialCharTok{$}\NormalTok{ENTREZID,}
                \AttributeTok{universe      =}\NormalTok{ background\_genes\_map}\SpecialCharTok{$}\NormalTok{ENTREZID,}
...
\FunctionTok{head}\NormalTok{(ego)}
\end{Highlighting}
\end{Shaded}
...
The code you see is used to find significantly enriched biological processes (BP) among a list of significant genes (\texttt{significant\_genes\_map\$ENTREZID}) compared to a background set of genes (\texttt{background\_genes\_map\$ENTREZID}). Here's what each parameter in the \texttt{enrichGO} function does:
...
Now, let's plot a barplot to visualize the enriched GO terms. The \texttt{ego} object contains the enrichment results generated previously. Each bar in the plot represents a GO term, and the height of the bar indicates the significance or enrichment level of that term. The \texttt{showCategory} parameter specifies how many top categories you want to display in the plot. In this case, we are displaying the top 20 enriched GO terms related to biological processes.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{barplot}\NormalTok{(ego, }\AttributeTok{showCategory=}\DecValTok{20}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-76-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{dotplot}\NormalTok{(ego)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-77-1.pdf}
...
\hypertarget{introduction-to-msigdb}{%
\subsection{Introduction to MSigDB}\label{introduction-to-msigdb}}

You can check the docs \href{https://www.gsea-msigdb.org/gsea/msigdb}{here}.

\texttt{MSigDB} (Molecular Signatures Database) is a widely used resource for gene set enrichment analysis. It provides curated collections of gene sets representing various biological processes, pathways, and functional categories. These gene sets are organized into different categories, making it easier to explore and analyze.
...
Let's start by loading the \texttt{msigdbr} package and accessing the MSigDB database. We'll specifically focus on the ``Hallmark gene sets'' (category H) for Homo sapiens (human).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the msigdbr package}
\FunctionTok{library}\NormalTok{(msigdbr)}

\CommentTok{\# Retrieve Hallmark gene sets for Homo sapiens}
\NormalTok{m\_df }\OtherTok{\textless{}{-}} \FunctionTok{msigdbr}\NormalTok{(}\AttributeTok{species =} \StringTok{"Homo sapiens"}\NormalTok{)}

\FunctionTok{head}\NormalTok{(m\_df)}
\end{Highlighting}
\end{Shaded}
...
The\texttt{m\_df} object now contains information about the Hallmark gene sets.

Let's use the \texttt{msigdbr} function with the species parameter set to ``Homo sapiens'' and the category parameter set to ``H'' to retrieve Hallmark gene sets. We then use the \texttt{dplyr::select} function to extract only the columns containing the gene set names (gs\_name) and associated entrez gene IDs (entrez\_gene).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m\_t2g }\OtherTok{\textless{}{-}} \FunctionTok{msigdbr}\NormalTok{(}\AttributeTok{species =} \StringTok{"Homo sapiens"}\NormalTok{, }\AttributeTok{category =} \StringTok{"H"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(gs\_name, entrez\_gene)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Count the number of genes in each gene set}
\FunctionTok{table}\NormalTok{(m\_t2g}\SpecialCharTok{$}\NormalTok{gs\_name)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{head}\NormalTok{(m\_t2g)}
\end{Highlighting}
\end{Shaded}
...
We'll use a set of significant genes from our analysis (represented by \texttt{significant\_genes\_map\$ENTREZID}) and the Hallmark gene sets (represented by \texttt{m\_t2g}). This analysis will help us identify which Hallmark gene sets are enriched in our significant genes.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{em }\OtherTok{\textless{}{-}} \FunctionTok{enricher}\NormalTok{(significant\_genes\_map}\SpecialCharTok{$}\NormalTok{ENTREZID, }\AttributeTok{TERM2GENE =}\NormalTok{ m\_t2g, }
               \AttributeTok{universe =}\NormalTok{ background\_genes\_map}\SpecialCharTok{$}\NormalTok{ENTREZID)}
\end{Highlighting}
\end{Shaded}

Here, we are using the \texttt{enricher} function from the \texttt{msigdbr} package to perform the enrichment analysis. It takes as input the list of significant genes, the mapping between gene sets and genes (\texttt{TERM2GENE}), and the background gene set (typically all detected genes in the dataset).

Now, let's examine the results of the enrichment analysis:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{head}\NormalTok{(em)}
\end{Highlighting}
\end{Shaded}
...
The \texttt{em} object contains information about the enriched gene sets, including their names, descriptions, p-values, and adjusted p-values (q-values). The q-value represents the corrected p-value and is often used to control for false discoveries in enrichment analysis.

To visualize the enrichment results, we can create a dotplot that shows the enriched gene sets and their significance.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{dotplot}\NormalTok{(em)}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-83-1.pdf}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Accessing Gene Sets: We use the \texttt{msigdbr} package to access gene sets from resources like \texttt{MSigDB}. These gene sets represent predefined pathways or functional categories.
\item
  Conversion of Gene Symbols: Similar to ORA, we convert gene symbols to Entrez IDs for consistency.
\item
...
\hypertarget{gsea-in-action}{%
\subsection{GSEA in action}\label{gsea-in-action}}

Before we dive into GSEA, we need to prepare our gene expression data. We have a dataset represented as res\_df, which contains information about genes, including their fold change and p-values. GSEA requires the data to be pre-ranked based on a metric that combines fold change and significance, such as signed fold change multiplied by the negative logarithm of the p-value.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res\_df }\OtherTok{\textless{}{-}}\NormalTok{ res\_df }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{signed\_rank\_stats =} \FunctionTok{sign}\NormalTok{(log2FoldChange) }\SpecialCharTok{*} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{left\_join}\NormalTok{(background\_genes\_map, }\AttributeTok{by =} \FunctionTok{c}\NormalTok{(}\StringTok{"gene"} \OtherTok{=} \StringTok{"SYMBOL"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{arrange}\NormalTok{(}\FunctionTok{desc}\NormalTok{(signed\_rank\_stats))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{res\_df }\OtherTok{\textless{}{-}}\NormalTok{ res\_df }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{negative\_log10pvalue =} \SpecialCharTok{{-}}\FunctionTok{log10}\NormalTok{(pvalue)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{negative\_log10pvalue =} \FunctionTok{ifelse}\NormalTok{(}\FunctionTok{is.infinite}\NormalTok{(negative\_log10pvalue), }\DecValTok{1000}\NormalTok{, negative\_log10pvalue)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{signed\_rank\_stats =} \FunctionTok{sign}\NormalTok{(log2FoldChange) }\SpecialCharTok{*}\NormalTok{ negative\_log10pvalue)}
\end{Highlighting}
\end{Shaded}
...
In the code below, we perform GSEA using our pre-ranked gene list and a reference gene set, represented by \texttt{m\_t2g}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{gene\_list }\OtherTok{\textless{}{-}}\NormalTok{ res\_df}\SpecialCharTok{$}\NormalTok{signed\_rank\_stats}
\FunctionTok{names}\NormalTok{(gene\_list) }\OtherTok{\textless{}{-}}\NormalTok{ res\_df}\SpecialCharTok{$}\NormalTok{ENTREZID}

\NormalTok{em2 }\OtherTok{\textless{}{-}} \FunctionTok{GSEA}\NormalTok{(gene\_list, }\AttributeTok{TERM2GENE =}\NormalTok{ m\_t2g)}

\FunctionTok{head}\NormalTok{(em2)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{em2}\SpecialCharTok{@}\NormalTok{result }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{View}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{visualizing-gsea-results}{%
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# save visualization in p1}

\NormalTok{p1}\OtherTok{\textless{}{-}} \FunctionTok{gseaplot}\NormalTok{(em2, }\AttributeTok{geneSetID =} \StringTok{"HALLMARK\_G2M\_CHECKPOINT"}\NormalTok{, }
              \AttributeTok{by =} \StringTok{"runningScore"}\NormalTok{, }\AttributeTok{title =} \StringTok{"HALLMARK\_G2M\_CHECKPOINT"}\NormalTok{)}
\NormalTok{p1}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-88-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# save visualization in p2}

\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{gseaplot}\NormalTok{(em2, }\AttributeTok{geneSetID =} \StringTok{"HALLMARK\_HYPOXIA"}\NormalTok{, }
               \AttributeTok{by =} \StringTok{"runningScore"}\NormalTok{, }\AttributeTok{title =} \StringTok{"HALLMARK\_HYPOXIA"}\NormalTok{)}

\NormalTok{p2}
\end{Highlighting}
\end{Shaded}

\includegraphics{11_Final_project_analyze_RNAseq_from_GEO_files/figure-latex/unnamed-chunk-88-2.pdf}

The X-axis is all your genes in the experiment (\textasciitilde{} 20,000 in this case) pre-ranked by our metric. Each black bar is the gene in this gene set(pathway). You have an idea where are the genes located in the pre-ranked list.
...
\item
  GSEA offers a broader perspective, considering the \textbf{entire} spectrum of gene expression to reveal more subtle shifts in gene sets related to particular biological themes.
\end{itemize}
...
\hypertarget{congratulations-for-successfully-completing-this-final-project}{%
\section{Congratulations for successfully completing this final project!}\label{congratulations-for-successfully-completing-this-final-project}}
...
\hypertarget{materials-to-read-to-uplift-your-r-skills-to-the-next-level}{%
\subsection{Materials to read to uplift your R skills to the next level}\label{materials-to-read-to-uplift-your-r-skills-to-the-next-level}}
...
\item
  \href{https://www.bigbookofr.com/index.html}{Big book of R}: The collection now stands at over 300 R books. Most of them are free.
\item
  \href{https://github.com/crazyhottommy/getting-started-with-genomics-tools-and-resources/blob/master/R_inferno.pdf}{R inferion}
\item
  \href{https://github.com/mcanouil/awesome-quarto}{Awesome Quarto}
\item
  \href{https://bioconductor.org/books/release/OSCA/}{Orchestrating Single-Cell Analysis with Bioconductor}. If you want to learn to use R for single-cell analysis. This is the book you should read.
\item
  \href{https://posit.co/resources/cheatsheets/}{Rstudio cheatsheets}
\end{itemize}

\hypertarget{coding-challenge}{%
\subsection{Coding Challenge}\label{coding-challenge}}

You have learned a lot! To challenge yourself after this course, you can try to finish this coding experience using TCGA data as well \url{https://github.com/crazyhottommy/coding_exercise_TCGA_infiltration}
